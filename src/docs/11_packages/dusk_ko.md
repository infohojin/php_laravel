...
> [!WARNING]
> Dusk의 서비스 제공자를 수동으로 등록하는 경우, **절대로** 운영 환경에서는 등록하지 않아야 하며, 그렇게 할 경우 임의의 사용자가 애플리케이션에 인증할 수 있게 될 위험이 있습니다.
...
Laravel Dusk가 `dusk:install` 명령을 통해 설치하는 것과 다른 버전의 ChromeDriver를 설치하고 싶다면, `dusk:chrome-driver` 명령을 사용할 수 있습니다:
...
> [!WARNING]
> Dusk는 `chromedriver` 바이너리를 실행 가능하도록 요구합니다. Dusk 실행에 문제가 있다면, 다음 명령어를 사용하여 바이너리가 실행 가능한지 확인해야 합니다: `chmod -R 0755 vendor/laravel/dusk/bin/`.
...
작성하는 대부분의 테스트는 애플리케이션 데이터베이스에서 데이터를 가져오는 페이지와 상호작용하지만, Dusk 테스트에서는 `RefreshDatabase` 트레잇을 절대 사용해서는 안 됩니다. `RefreshDatabase` 트레잇은 HTTP 요청 간에 적용되거나 사용 가능하지 않은 데이터베이스 트랜잭션을 활용합니다. 대신 두 가지 옵션이 있습니다: `DatabaseMigrations` 트레잇과 `DatabaseTruncation` 트레잇입니다.

<a name="reset-migrations"></a>
#### 데이터베이스 마이그레이션 사용하기

`DatabaseMigrations` 트레잇은 각 테스트 전에 데이터베이스 마이그레이션을 실행합니다. 그러나 각 테스트마다 데이터베이스 테이블을 삭제하고 다시 만드는 것은 일반적으로 테이블을 단순히 초기화하는 것보다 느립니다:```php tab=Pest
<?php
...
> [!NOTE]
> Pest를 사용하는 경우, 기본 `DuskTestCase` 클래스나 테스트 파일이 확장하는 클래스에 속성이나 메서드를 정의해야 합니다.
...
마지막으로 `dusk` 명령을 실행했을 때 테스트가 실패했다면, `dusk:fails` 명령을 사용하여 실패한 테스트를 먼저 다시 실행함으로써 시간을 절약할 수 있습니다:
...
`dusk` 명령은 Pest / PHPUnit 테스트 러너에서 일반적으로 허용되는 모든 인수를 받아들이며, 예를 들어 주어진 [그룹](https://docs.phpunit.de/en/10.5/annotations.html#group)의 테스트만 실행하도록 허용할 수 있습니다:
...
> [!NOTE]
> 로컬 개발 환경을 관리하기 위해 [Laravel Sail](/docs/{{version}}/sail)을 사용하고 있다면, [Dusk 테스트 구성 및 실행](/docs/{{version}}/sail#laravel-dusk)에 대한 Sail 문서를 참조하십시오.
...
또한, 9515가 아닌 다른 포트에서 ChromeDriver를 시작하는 경우, 동일한 클래스의 `driver` 메서드를 올바른 포트를 반영하도록 수정해야 합니다:
...
시작하려면, 우리 애플리케이션에 로그인할 수 있는지 검증하는 테스트를 작성해 봅시다. 테스트를 생성한 후에는 로그인 페이지로 이동하고, 일부 자격 증명을 입력하고, "로그인" 버튼을 클릭하도록 수정할 수 있습니다. 브라우저 인스턴스를 생성하려면 Dusk 테스트 내에서 `browse` 메서드를 호출하면 됩니다:```php tab=Pest
<?php
...
위의 예제에서 볼 수 있듯이, `browse` 메서드는 클로저를 허용합니다. 브라우저 인스턴스는 Dusk에 의해 자동으로 이 클로저에 전달되며, 이는 애플리케이션과 상호 작용하고 검증(assertion)을 수행하는 데 사용되는 주요 객체입니다.

<a name="creating-multiple-browsers"></a>
#### 여러 브라우저 생성하기

때때로 테스트를 제대로 수행하기 위해 여러 브라우저가 필요할 수 있습니다. 예를 들어, 웹소켓과 상호 작용하는 채팅 화면을 테스트하기 위해 여러 브라우저가 필요할 수 있습니다. 여러 브라우저를 생성하려면 `browse` 메서드에 전달된 클로저의 시그니처에 브라우저 인자를 더 추가하면 됩니다:
...
브라우저 창의 크기를 조정하기 위해 `resize` 방법을 사용할 수 있습니다:
...
`fitContent` 메서드는 브라우저 창의 크기를 내용 크기에 맞게 조정합니다:
...
브라우저 창을 화면의 다른 위치로 이동시키기 위해 `move` 방법을 사용할 수 있습니다:
...
다음 쿠키를 삭제하려면 `deleteCookie` 방법을 사용할 수 있습니다:
...
주어진 파일 이름으로 스크린샷을 찍고 저장하려면 `screenshot` 방법을 사용할 수 있습니다. 모든 스크린샷은 `tests/Browser/screenshots` 디렉토리에 저장됩니다:
...
`clear` 메서드를 사용하여 입력 값의 값을 지울 수 있습니다:
...
`select` 요소에서 사용 가능한 값을 선택하려면 `select` 메서드를 사용할 수 있습니다. `type` 메서드와 마찬가지로 `select` 메서드는 전체 CSS 선택자를 필요로 하지 않습니다. `select` 메서드에 값을 전달할 때는 표시 텍스트 대신 기본 옵션 값을 전달해야 합니다:
...
체크박스 입력을 "체크"하려면 `check` 방법을 사용할 수 있습니다. 다른 입력 관련 방법들과 마찬가지로, 전체 CSS 선택자가 필요하지 않습니다. CSS 선택자 일치를 찾을 수 없는 경우, Dusk는 일치하는 `name` 속성을 가진 체크박스를 검색합니다:
...
`attach` 메서드는 파일을 `file` 입력 요소에 첨부하는 데 사용될 수 있습니다. 다른 많은 입력 관련 메서드와 마찬가지로 전체 CSS 선택자가 필요하지 않습니다. CSS 선택자 매치가 발견되지 않으면, Dusk는 일치하는 `name` 속성을 가진 `file` 입력을 검색합니다:
...
양식을 제출할 때, 많은 애플리케이션은 버튼이 눌린 후 양식 제출 버튼을 비활성화하고 양식 제출 HTTP 요청이 완료되면 버튼을 다시 활성화합니다. 버튼을 누르고 버튼이 다시 활성화될 때까지 기다리려면 `pressAndWaitFor` 방법을 사용할 수 있습니다:
...
주어진 표시 텍스트를 가진 링크가 페이지에 표시되는지 확인하려면 `seeLink` 방법을 사용할 수 있습니다:
...
Dusk는 또한 `withKeyboard` 메서드를 제공하여 `Laravel\Dusk\Keyboard` 클래스를 통해 복잡한 키보드 상호작용을 유창하게 수행할 수 있습니다. `Keyboard` 클래스는 `press`, `release`, `type` 및 `pause` 메서드를 제공합니다:
...
`doubleClick` 방법은 마우스의 더블 클릭을 시뮬레이션하는 데 사용될 수 있습니다:
...
`clickAndHold` 메서드는 마우스 버튼이 눌린 상태로 클릭되는 것을 시뮬레이션하는 데 사용될 수 있습니다. 이어서 `releaseMouse` 메서드를 호출하면 이 동작이 취소되고 마우스 버튼이 해제됩니다:
...
JavaScript 대화상자에 프롬프트가 포함된 경우 `typeInDialog` 메서드를 사용하여 프롬프트에 값을 입력할 수 있습니다:
...
가끔 현재 범위 밖에서 어설션을 실행해야 할 수도 있습니다. 이를 수행하기 위해 `elsewhere` 및 `elsewhereWhenAvailable` 메서드를 사용할 수 있습니다:
...
단순히 주어진 밀리초 동안 테스트를 일시 중지해야 하는 경우에는 `pause` 메서드를 사용하세요:
...
`waitFor` 메서드는 지정된 CSS 또는 Dusk 선택자와 일치하는 요소가 페이지에 표시될 때까지 테스트 실행을 일시 중지하는 데 사용할 수 있습니다. 기본적으로, 이 메서드는 예외를 발생시키기 전 최대 5초 동안 테스트를 일시 중지합니다. 필요하다면, 메서드의 두 번째 인수로 사용자 지정 시간 제한을 전달할 수 있습니다:
...
때때로, 주어진 선택자와 일치하는 요소가 나타날 때까지 기다린 후 해당 요소와 상호작용하고 싶을 수 있습니다. 예를 들어, 모달 창이 활성화될 때까지 기다린 다음 모달 내의 "확인" 버튼을 누르고 싶을 수 있습니다. `whenAvailable` 메서드를 사용하여 이를 수행할 수 있습니다. 주어진 클로저 내에서 수행되는 모든 요소 작업은 원래 선택자에 범위가 지정됩니다:
...
`waitForText` 메서드는 페이지에 지정된 텍스트가 표시될 때까지 기다리는 데 사용할 수 있습니다:
...
`$browser->assertPathIs('/home')`와 같은 경로 단언을 수행할 때, `window.location.pathname`가 비동기적으로 업데이트되는 경우 단언이 실패할 수 있습니다. 위치가 특정 값이 될 때까지 기다리기 위해 `waitForLocation` 메서드를 사용할 수 있습니다:
...
`waitForEvent` 메서드에 이벤트 리스너를 특정 요소에 붙이기 위해 두 번째 인수로 선택자를 제공할 수도 있습니다:
...
`document` 및 `window` 객체의 이벤트를 기다릴 수도 있습니다:
...
때때로 브라우저의 보기 가능한 영역 밖에 요소가 있어서 클릭할 수 없을 수 있습니다. `scrollIntoView` 메서드는 지정된 선택자에 있는 요소가 뷰 안에 들어올 때까지 브라우저 창을 스크롤합니다:
...
주어진 드롭다운이 지정된 값이 선택되지 않았음을 확인합니다:
...
주어진 선택자와 일치하는 요소에 제공된 속성이 없는지 확인합니다:
...
주어진 선택자와 일치하는 요소가 제공된 데이터 속성에 주어진 값을 가지고 있는지 확인합니다:
...
때때로 테스트에서는 여러 복잡한 동작을 순차적으로 수행해야 할 때가 있습니다. 이는 테스트를 읽고 이해하기 어렵게 만들 수 있습니다. Dusk 페이지를 사용하면 특정 페이지에서 단일 메서드를 통해 수행할 수 있는 표현력 있는 동작을 정의할 수 있습니다. 페이지는 또한 애플리케이션이나 단일 페이지에 대한 일반 선택자에 대한 바로 가기를 정의할 수 있습니다.

<a name="generating-pages"></a>
### 페이지 생성

페이지 객체를 생성하려면 `dusk:page` Artisan 명령을 실행하십시오. 모든 페이지 객체는 애플리케이션의 `tests/Browser/Pages` 디렉터리에 배치됩니다:
...
Dusk를 설치한 후, 기본 `Page` 클래스가 귀하의 `tests/Browser/Pages` 디렉토리에 배치됩니다. 이 클래스에는 애플리케이션 내 모든 페이지에서 사용 가능해야 하는 전역 축약 선택자를 정의하는 데 사용할 수 있는 `siteElements` 메서드가 포함되어 있습니다:
...
페이지에 정의된 기본 메서드 외에도, 테스트 전반에서 사용할 수 있는 추가 메서드를 정의할 수 있습니다. 예를 들어, 음악 관리 애플리케이션을 만들고 있다고 가정해 봅시다. 애플리케이션의 한 페이지에서 흔히 수행하는 동작 중 하나는 플레이리스트를 만드는 것입니다. 각 테스트에서 플레이리스트를 만드는 로직을 다시 작성하는 대신, 페이지 클래스에 `createPlaylist` 메서드를 정의할 수 있습니다:
...
구성 요소는 Dusk의 "페이지 객체"와 유사하지만, 애플리케이션 전반에 걸쳐 재사용되는 UI 및 기능 조각(예: 네비게이션 바나 알림 창)에 사용됩니다. 따라서 구성 요소는 특정 URL에 제한되지 않습니다.

<a name="generating-components"></a>
### 구성 요소 생성

구성 요소를 생성하려면 `dusk:component` Artisan 명령어를 실행합니다. 새 구성 요소는 `tests/Browser/Components` 디렉토리에 배치됩니다:
...
[Dusk 테스트](https://github.com/features/actions)를 실행하기 위해 GitHub Actions를 사용하는 경우, 다음 구성 파일을 시작점으로 사용할 수 있습니다. TravisCI와 마찬가지로, PHP의 내장 웹 서버를 실행하기 위해 `php artisan serve` 명령을 사용할 것입니다:
...
[Dusk 테스트](https://chipperci.com)를 실행하기 위해 [Chipper CI](https://chipperci.com)를 사용하고 있다면, 다음 구성 파일을 시작점으로 사용할 수 있습니다. 요청을 수신할 수 있도록 Laravel을 실행하기 위해 PHP의 내장 서버를 사용할 것입니다:
...
Chipper CI에서 Dusk 테스트를 실행하는 방법, 데이터베이스 사용법을 포함하여 자세히 알아보려면 [공식 Chipper CI 문서](https://chipperci.com/docs/testing/laravel-dusk-new/)를 참조하세요.
{% endraw %}
